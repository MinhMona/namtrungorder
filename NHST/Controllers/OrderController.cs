...
namespace NHST.Controllers
{
    public class OrderController
    {
        ...
        public static string Insert(int UID, string title_origin, string title_translated, string price_origin, string price_promotion, string property_translated, string property, string data_value,
            ...
            string Deposit, string CurrentCNYVN, string TotalPriceVND, int MainOrderID, DateTime CreatedDate, int CreatedBy, string Link)
        {
        ...
        public static string UpdateLink(int ID, string Link, bool getLink)
        {
            using (var dbe = new NHSTEntities())
            {
                var op = dbe.tbl_Order.Where(o => o.ID == ID).FirstOrDefault();
                if (op != null)
                {
                    op.link = Link;
                    op.isGetLink = getLink;
                    dbe.Configuration.ValidateOnSaveEnabled = false;
        ...
        public static List<tbl_Order> GetByMainOrderID(int MainOrderID)
        {
            ...
            {
                List<tbl_Order> lo = new List<tbl_Order>();
                lo = dbe.tbl_Order.Where(o => o.MainOrderID == MainOrderID).OrderBy(o => o.ID).ToList();
                return lo;
            }
        }
        public static int GetCountByMainOrderID_SQL(int MainOrderID)
        {
            int Count = 0;
            var sql = @"SELECT COUNT(*) as Total from tbl_Order as mo";
